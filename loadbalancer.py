            ...
            if len(args) == 2:
                ...
                for i in range(N_WORKER):
                    if LOAD_DICT[i] < LOAD_DICT[minLoad]:
                        minLoad = i
                r = requests.get(WORKER_DICT[minLoad][IP] + ":" + str(WORKER_DICT[minLoad][PORT]) + "/" + args[2],
                                 timeout=0.01)
                ...
                if n == "voteRequest":
                    ...
                    if not IS_LEADER:
                        ...
                        if countCommit == leaderCommit - 1:
                            ...
                            try:
                                ...
                                r = requests.get("{}:{}/positive/{}/{}".format(NODE_DICT[id_leader][IP],
                                                                               str(NODE_DICT[id_leader][PORT]),
                                                                               countCommit + 1, str(ID), timeout=0.01))
                            except:
                                pass
                                # print(e)
                            ...
                            id_leader = int(args[2])
                            try:
                                r = requests.get(NODE_DICT[id_leader][IP] + ":" + str(
                                    NODE_DICT[id_leader][PORT]) + "/negative/" + str(countCommit + 1) + "/" + str(ID),
                                                 timeout=0.01)
                            except:
                                pass
                                # print(e)
                    ...
                    if countCommit == commitLine - 1:
                        ...
                        r = requests.get(
                            NODE_DICT[id_leader][IP] + ":" + str(NODE_DICT[id_leader][PORT]) + "/positivecommit/" +
                            args[3] + "/" + str(ID), timeout=0.01)

                    elif countCommit < commitLine - 1:
                        try:
                            r = requests.get(
                                NODE_DICT[id_leader][IP] + ":" + str(NODE_DICT[id_leader][PORT]) + "/negativecommit/" +
                                args[3] + "/" + str(ID), timeout=0.01)
...
def sendCPULoad(ip, port):
    try:
        ...
        r = requests.get(ip + ":" + port + "/data/" + str(ID) + "/" + str(TERM) + "/" + str(COMMIT_DICT[ID] + 1),
