    ...
    def do_GET(self):
        ...
        try:
            ...
            if len(args) == 2:
                ...
                if min_load >= 0:
                    r = requests.get(
                        "{worker_ip}:{worker_port}/{number}".format(worker_ip=WORKER_DICT[min_load][IP],
                                                                    worker_port=WORKER_DICT[min_load][
                ...
                if n == "voteRequest":
                    ...
                    if not IS_LEADER:
                        if (int(args[3]) >= TERM):
                            ...
                            try:
                                ...
                                r = requests.get("{node_ip}:{node_port}/positive/{term}/{index}/{node_id}".format(
                                    node_ip=NODE_DICT[id_leader][IP],
                                    node_port=NODE_DICT[id_leader][PORT],
                                    term=TERM, index=META_DATA[DATA_INDEX], node_id=ID), timeout=0.01)
                            except:
                                pass
                        else:
                            ...
                            id_leader = int(args[2])
                            try:
                                r = requests.get("{node_ip}:{node_port}/negative/{term}/{last_commit}/{node_id}".format(
                                    node_ip=NODE_DICT[id_leader][IP], node_port=NODE_DICT[id_leader][PORT],
                                    term=TERM, last_commit=getLastCommit(), node_id=ID),
                                    timeout=0.01)
                            except:
                                pass
                    ...
                    if not IS_LEADER:
                        ...
                        if (int(args[3]) >= META_DATA[DATA_TERM] and int(args[4].split('?')[0]) >= META_DATA[
                            DATA_INDEX]):
                            ...
                            try:
                                id_leader = int(args[2])
                                print("# NODE FOLLOWER : SENDING POSITIVE BACK TO THE LEADER")
                                r = requests.get("{node_ip}:{node_port}/positive/{commit}/{term}/{node_id}".format(
                                    node_ip=NODE_DICT[id_leader][IP], node_port=NODE_DICT[id_leader][PORT],
                                    commit=META_DATA[DATA_TERM], term=META_DATA[DATA_INDEX], node_id=ID), timeout=0.01)
                            except:
                                pass
                            ...
                            id_leader = int(args[2])
                            try:
                                r = requests.get("{node_ip}:{node_port}/negativecommit/{term}/{last_commit}/{node_id}".format(
                                    node_ip=NODE_DICT[id_leader][IP], node_port=NODE_DICT[id_leader][PORT],
                                    term=TERM, last_commit=getLastCommit(), node_id=ID), timeout=0.01)
                            except:
                                pass
                    ...
                    if IS_LEADER:
                        ...
                        if int(workerid) in WORKER_DICT:
                            ...
                            print("Sorry the worker {id} is not defined.".format(id=workerid))
